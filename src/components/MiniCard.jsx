...
const MiniCard = ({ time, temp, iconString }) => {
  const [icon, setIcon] = useState(); // Holds the correct weather icon

  // Decide which icon to use when condition changes
  useEffect(() => {
    if (iconString) {
  ...
  return (
    <div className='glassCard w-[10rem] h-[10rem] p-4 flex flex-col'> 
      <p className='text-center'>
        {
          new Date(time)
            .toLocaleTimeString('en', { weekday: 'long' }) // Convert to readable weekday
            .split(" ")[0]
        }
      </p>
      <hr/>
